...
public class SizeAndAgeBatchingTest {
    ...
    public void testQueueFull() throws Exception {
        ...
        while(q.getCurrentBatch().events.size() < subscriberConfig.getBatchSize()) {
            q.offer("EventToFill");
        }
    ...
    private AgeBatchingQueue newQ(Method subMethod, SubscriberConfigProvider.SubscriberConfig annotation) {
        return new SizeAndAgeBatchingQueue(subMethod, annotation, false, new AtomicLong());
    }

    public class MySub {

        @Subscribe(batchAge = 1000, queueSize = 2, batchSize = 2, batchingStrategy = Subscribe.BatchingStrategy.Age)
        public void subMe(String event) {
            System.out.println("AgeBatchingTest$MySub.subMe");
        }

        @Subscribe(batchAge = 60000, queueSize = 2, batchSize = 2, batchingStrategy = Subscribe.BatchingStrategy.Age)
